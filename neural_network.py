...
def get_x_y_arrays(img_data_save_path, label_data_save_path, n=50):
    ...
    symbol_dict = {}
    # Currently looping through all images twice; can fix later
...
def train_test_val_split(X, y, fracs=(0.7, 0.1, 0.2)):
    # TODO: add stratification param? Change random state?
    """
    ...
    filename = "output/split_data_" + date.today().strftime("%m%d%y")
    np.savez_compressed(
            filename,
            x_train=x_train,
            x_test=x_test,
...
def train(X, y, model_path):
    ...
    model.summary()
    # can't get tf.keras.metrics.SparseTopKCategoricalAccuracy in tf 1.13
